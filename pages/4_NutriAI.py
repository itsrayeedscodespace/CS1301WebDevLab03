...
if st.button("Get Advice"):
    if query:
        with st.spinner("Analyzing nutritional information..."):
            # Get recipe overview if available
            if recipe_overview:
            ...
            if nutrition_query:
                ...
                # Clear previous messages and add new initial message
                st.session_state.messages = []
                st.session_state.messages.append({"role": "assistant", "content": advice})
                st.session_state.initial_advice_given = True
            
            # Show similar recipes if available
...
# Only show chat interface if we have ingredients and initial advice has been given
if st.session_state.current_ingredients and st.session_state.initial_advice_given:
    ...
    # Add default message if it's the first interaction
